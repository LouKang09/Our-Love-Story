...
document.addEventListener('touchstart',e=>{
  ...
  if(touchY<=phonePullTopBoundary()+4){phonePullReset();return;}
...
async function loadMyDays(){
  if(!me?.tag)return;
  const previous=myDayCurrent();
  const previousTag=previous.group?.profile?.tag || '';
  ...
  if(previousTag && !$('#myDayViewer')?.classList.contains('hidden')){
    ...
    if(nextGroupIndex>=0){
      myDayGroupIndex=nextGroupIndex;
      const nextItems=myDayGroups[nextGroupIndex].items || [];
      const nextItemIndex=nextItems.findIndex(item=>item.id===previousStoryId);
      myDayItemIndex=nextItemIndex>=0 ? nextItemIndex : Math.min(myDayItemIndex,Math.max(0,nextItems.length-1));
    }
  }
  renderMyDayTray();
...
function renderMyDayTray(){
  ...
  add.innerHTML=
    '<span class="my-day-create-visual">'+avatarHtml(me,'my-day-card-avatar')+'<b>＋</b></span>'+
    '<strong>Create story</strong>';

  const own=myDayGroups.find(group=>group.own && group.items?.length);
  const groups=[
    ...(own?[own]:[]),
    ...myDayGroups.filter(group=>!group.own && group.items?.length)
  ];
  ...
  if(isNativeScrapellaApp()){
    ...
    if(camera?.getPhoto){
      try{
        ...
        showToast('Opening story camera…');
...
function activeStoryTextBox(){
  return pendingStoryTextBoxes.find(box=>box.id===activeStoryTextBoxId)||null;
}
function storyTextElementById(id){
  const layer=$('#storyTextLayer');
  if(!layer||!id)return null;
  return [...layer.querySelectorAll('[data-story-text-id]')].find(el=>el.dataset.storyTextId===id)||null;
}
function syncActiveStoryTextState(){
...
function syncActiveStoryTextBoxFromElement(){
  ...
  box.text=String(overlay.innerText ?? overlay.textContent ?? '').replace(/\u00a0/g,' ').slice(0,180);
...
function storyComposerRawText(){
  const overlay=storyTextElementById(activeStoryTextBoxId);
  if(overlay)return String(overlay.innerText ?? overlay.textContent ?? '').replace(/\u00a0/g,' ').slice(0,180);
  return String(activeStoryTextBox()?.text||'').slice(0,180);
}
function storyComposerText(){
  return storyComposerRawText().trim();
}
function storyComposerTextBoxes(){
  syncActiveStoryTextBoxFromElement();
  return pendingStoryTextBoxes.map((box,index)=>({
...
function storyItemTextBoxes(item){
  const boxes=Array.isArray(item?.textBoxes)?item.textBoxes:[];
  const normalized=boxes.slice(0,12).map((box,index)=>({
...
function syncStoryTextBoxElement(box){
  ...
  let overlay=storyTextElementById(box.id);
  if(!overlay){
    overlay=document.createElement('div');
    overlay.className='my-day-overlay-text story-editable-text';
    overlay.dataset.storyTextId=box.id;
    overlay.dataset.editing='0';
    overlay.contentEditable='false';
    overlay.setAttribute('role','textbox');
    overlay.setAttribute('aria-label','Story text');
    overlay.dataset.placeholder='Type or @tag…';
...
function renderStoryTextBoxes(){
  const layer=$('#storyTextLayer');
  if(!layer)return;
  const ids=new Set(pendingStoryTextBoxes.map(box=>box.id));
  layer.querySelectorAll('[data-story-text-id]').forEach(el=>{if(!ids.has(el.dataset.storyTextId))el.remove();});
...
function renderStoryInlineMentionSuggestions(){
  ...
  host.style.left=storyTextCoordinate(box.x,50)+'%';
  if(box.y>56){host.style.top='auto';host.style.bottom=Math.max(8,100-storyTextCoordinate(box.y,50)+7)+'%';}
  else{host.style.bottom='auto';host.style.top=Math.min(86,storyTextCoordinate(box.y,50)+8)+'%';}
...
function finishStoryTextEditing({removeEmpty=true}={}){
  const box=activeStoryTextBox();
  const overlay=storyTextElementById(activeStoryTextBoxId);
  if(!box||!overlay){hideStoryInlineMentionSuggestions();return;}
  syncActiveStoryTextBoxFromElement();
  overlay.dataset.editing='0';
  overlay.contentEditable='false';
  overlay.blur();
  if(removeEmpty&&!String(box.text||'').trim()){
    pendingStoryTextBoxes=pendingStoryTextBoxes.filter(entry=>entry.id!==box.id);
    overlay.remove();
...
function startStoryTextEditing({create=false,id=''}={}){
  ...
  if(create){
    ...
    if(activeStoryTextBoxId&&activeStoryTextBoxId!==id)finishStoryTextEditing();
    activeStoryTextBoxId=id;
  }else if(!activeStoryTextBoxId){
    if(pendingStoryTextBoxes.length)activeStoryTextBoxId=pendingStoryTextBoxes[0].id;
    else return startStoryTextEditing({create:true});
  }
  const box=activeStoryTextBox();
  if(!box)return;
...
function appendStoryComposerText(value){
  if(!activeStoryTextBoxId)startStoryTextEditing({create:true});
  const box=activeStoryTextBox();
  const overlay=storyTextElementById(activeStoryTextBoxId);
  if(!box||!overlay)return;
...
function setStoryTextFromPointer(event){
  ...
  box.x=pendingStoryTextX;box.y=pendingStoryTextY;
  syncMyDayComposerText();
}
...
function setMyDayPostProgress(percent,label,{state='working'}={}){
  ...
  const fill=$('#myDayPostProgressFill');
  const percentEl=$('#myDayPostProgressPercent');
  const labelEl=$('#myDayPostProgressLabel');
  ...
  box.style.setProperty('--myday-post-progress',String(value/100));
  if(fill)fill.style.width=value+'%';
  if(percentEl)percentEl.textContent=value+'%';
  if(labelEl)labelEl.textContent=label||'Posting story…';
}
function resetMyDayPostProgress(){
  const box=$('#myDayPostProgress');
  box?.classList.add('hidden');
  box?.classList.remove('is-done','is-error');
  box?.style.setProperty('--myday-post-progress','0');
  if($('#myDayPostProgressFill'))$('#myDayPostProgressFill').style.width='0%';
  if($('#myDayPostProgressPercent'))$('#myDayPostProgressPercent').textContent='0%';
  if($('#myDayPostProgressLabel'))$('#myDayPostProgressLabel').textContent='Preparing story…';
}
function myDayUploadRequest(payload,onProgress){
  return new Promise((resolve,reject)=>{
    ...
    xhr.onerror=()=>reject(new Error('The My Day photo could not be uploaded. Check your connection and try again.'));
    xhr.onload=()=>{
      let data={};
      try{data=JSON.parse(xhr.responseText||'{}');}catch{}
      if(xhr.status>=200&&xhr.status<300)resolve(data);
      else reject(new Error(data.error||'The My Day photo could not be uploaded.'));
    };
    xhr.send(JSON.stringify(payload));
  });
}
async function uploadMyDayImageWithProgress(file){
  setMyDayPostProgress(4,'Preparing photo…');
  const compressed=await compressImageForUpload(file);
  if(compressed.size>8*1024*1024)throw new Error((file.name||'Photo')+' is larger than 8 MB after compression.');
  setMyDayPostProgress(22,'Preparing photo…');
...
function prepareMyDayFile(file,{openText=false}={}){
  ...
  document.body.classList.add('my-day-open');
  if(openText)setTimeout(()=>startStoryTextEditing({create:true}),180);
}
...
async function postMyDay(){
  ...
  const button=$('#myDayPostBtn');
  const retake=$('#myDayRetakeBtn');
  const close=$('#myDayComposerClose');
  myDayPosting=true;
  ...
  try{
    ...
    const storyTextBoxes=storyComposerTextBoxes();
    const firstStoryText=storyTextBoxes[0]||null;
    const posted=await api('/api/my-day',{method:'POST',body:JSON.stringify({
      image:uploaded.src,
      overlayText:firstStoryText?.text||'',
    ...
    myDayPosting=false;
    setMyDayPostProgress(0,'Posting failed · tap Post to retry',{state:'error'});
    showToast(err?.message||'Could not post My Day.');
  }finally{
    if(button){button.disabled=false;button.textContent='Share to story';}
    if(retake)retake.disabled=false;
    if(close)close.disabled=false;
  }
}
async function markCurrentMyDayViewed(){
  const current=myDayCurrent(),group=current.group,item=current.item;
  if(!group||!item||group.profile?.tag===me?.tag)return;
  try{await api('/api/my-day/'+encodeURIComponent(item.id)+'/view',{method:'POST',body:'{}'});}catch{}
}
function stopMyDayStoryTimer(){
...
function startMyDayStoryTimer(storyId){
  ...
  const tick=now=>{
    ...
    if(fraction>=1){
      myDayStoryFrame=0;
      advanceMyDayStory(1,{automatic:true});
      return;
    }
    myDayStoryFrame=requestAnimationFrame(tick);
  };
...
function ensureMyDayStoryTimer(storyId){
  if(!storyId)return;
  if(myDayTimedStoryId===storyId && myDayStoryFrame)return;
  startMyDayStoryTimer(storyId);
}
function advanceMyDayStory(delta,{automatic=false}={}){
  if(!myDayGroups.length)return closeMyDayViewer();
  stopMyDayStoryTimer();
  let groupIndex=myDayGroupIndex;
  let itemIndex=myDayItemIndex;
  if(delta>0){
    const currentItems=myDayGroups[groupIndex]?.items||[];
    ...
    if(itemIndex>0){
      ...
      groupIndex-=1;
      while(groupIndex>=0 && !(myDayGroups[groupIndex]?.items?.length))groupIndex-=1;
      if(groupIndex<0){
        startMyDayStoryTimer(myDayCurrent().item?.id);
        return;
      }
      itemIndex=Math.max(0,(myDayGroups[groupIndex].items?.length||1)-1);
    }
  }
  myDayGroupIndex=groupIndex;
  myDayItemIndex=itemIndex;
...
function renderMyDayViewer(){
  ...
  $('#myDayProgress').innerHTML=group.items.map((_,index)=>{
    const state=index<myDayItemIndex?'done':index===myDayItemIndex?'current':'pending';
    const width=index<myDayItemIndex?'100%':'0%';
  ...
  const viewerTextLayer=$('#myDayViewerTextLayer');
  if(viewerTextLayer){
    viewerTextLayer.innerHTML='';
    storyItemTextBoxes(item).forEach(box=>{
  ...
  if(counter){
    counter.textContent=multiple ? (String(myDayItemIndex+1)+' of '+String(group.items.length)) : '';
    counter.classList.toggle('hidden',!multiple);
  }
  const prev=$('#myDayPrevBtn'),next=$('#myDayNextBtn');
  if(prev){
    prev.classList.toggle('hidden',!multiple||myDayItemIndex<=0);
    prev.disabled=myDayItemIndex<=0;
  }
  if(next){
...
storyTextLayer?.addEventListener('click',event=>{
  ...
  const id=String(overlay.dataset.storyTextId||'');
  if(!id)return;
  if(id!==activeStoryTextBoxId||overlay.dataset.editing!=='1')startStoryTextEditing({id});
});
storyTextLayer?.addEventListener('pointerdown',event=>{
  const overlay=event.target?.closest?.('[data-story-text-id]');
  if(!overlay)return;
  const id=String(overlay.dataset.storyTextId||'');
...
$('#storyEditorTextBtn')?.addEventListener('click',()=>startStoryTextEditing({create:true}));
...
function wireMemoryConstellationPan(){
  ...
  const minScale=isPhoneUI()?0.35:0.48;
  const maxScale=2.2;
...
let universeCategoryActionPopover=null;
let universeCategoryActionOutsideHandler=null;
function closeUniverseCategoryActionPopover(){
  if(universeCategoryActionPopover){
    universeCategoryActionPopover.remove();
    universeCategoryActionPopover=null;
  }
  if(universeCategoryActionOutsideHandler){
    document.removeEventListener('pointerdown',universeCategoryActionOutsideHandler,true);
    universeCategoryActionOutsideHandler=null;
  }
}
function showUniverseCategoryActionPopover(mode,anchor,point=null){
  if((universeTargetTag || me?.tag)!==me?.tag || !OFFICIAL_UNIVERSE_MODES.includes(mode))return;
  closeUniverseCategoryActionPopover();
  const pop=document.createElement('div');
  pop.className='universe-category-action-popover';
  pop.setAttribute('role','menu');
  pop.innerHTML=`<button type="button" role="menuitem" data-hide-universe-popover><span aria-hidden="true">⊘</span> Hide</button>`;
  document.body.appendChild(pop);
  universeCategoryActionPopover=pop;
  requestAnimationFrame(()=>{
    const anchorRect=anchor?.getBoundingClientRect?.() || {left:window.innerWidth/2,top:window.innerHeight/2,bottom:window.innerHeight/2,width:0};
    const rect=pop.getBoundingClientRect();
    const px=Number(point?.x);
    const py=Number(point?.y);
    let left=Number.isFinite(px)?px-rect.width/2:anchorRect.left+(anchorRect.width-rect.width)/2;
    let top=Number.isFinite(py)?py+10:anchorRect.bottom+7;
    left=Math.max(8,Math.min(window.innerWidth-rect.width-8,left));
    if(top+rect.height>window.innerHeight-8)top=Math.max(8,anchorRect.top-rect.height-7);
    pop.style.left=`${left}px`;
    pop.style.top=`${top}px`;
  });
  pop.querySelector('[data-hide-universe-popover]')?.addEventListener('click',async e=>{
    e.preventDefault();
    e.stopPropagation();
    closeUniverseCategoryActionPopover();
    await hideUniverseCategory(mode);
  });
  setTimeout(()=>{
    if(universeCategoryActionPopover!==pop)return;
    universeCategoryActionOutsideHandler=e=>{
      if(!pop.contains(e.target) && e.target!==anchor)closeUniverseCategoryActionPopover();
    };
    document.addEventListener('pointerdown',universeCategoryActionOutsideHandler,true);
  },0);
}
function wireUniverseCategoryLongPress(btn,mode,enabled){
  ...
  btn.onpointerdown=e=>{
    ...
    timer=setTimeout(()=>{
      longPressed=true;
      btn._suppressUniverseClick=true;
      showUniverseCategoryActionPopover(mode,btn,point);
      if(navigator.vibrate)navigator.vibrate(18);
    },430);
  };
  btn.onpointermove=e=>{
    if(Math.hypot(e.clientX-startX,e.clientY-startY)>13)clearTimeout(timer);
  };
  const finish=()=>{
    clearTimeout(timer);
    if(longPressed)setTimeout(()=>{btn._suppressUniverseClick=false;},520);
  };
  btn.onpointerup=finish;
  btn.onpointercancel=finish;
  btn.oncontextmenu=e=>{
    e.preventDefault();
    btn._suppressUniverseClick=true;
    showUniverseCategoryActionPopover(mode,btn,{x:e.clientX,y:e.clientY});
    setTimeout(()=>{btn._suppressUniverseClick=false;},520);
  };
}
...
function renderUniverseLabs(){
  ...
  document.querySelectorAll('.universe-lab-tab-item').forEach(item=>{
    ...
    btn.onclick=official && !hidden ? (e=>{
      if(btn._suppressUniverseClick){
        e?.preventDefault?.();
        btn._suppressUniverseClick=false;
        return;
      }
      universeLabSetMode(mode);
    }) : null;
